...
public class ImageService {
    ...
    @Autowired private DrawerService drawerService;

    public SecondaryStructureImage provideVisualization(
            VisualizationTool visualizationTool, DotBracket combinedStrand) {
        ...
        final SecondaryStructureDrawer backupDrawer =
                drawerService.loadDrawer(visualizationTool.getBackupVisualizationTool());

        // TODO: restore the cache
        //  probably it would be best to use Spring cache mechanism since we're embedding
        // rnapdbee-common into engine
        final ImageCache imageCache = new ImageCacheImpl();

        try {
            SecondaryStructureImage image =
                    RNApdbeeDrawer.drawCanonical(
                            imageCache, mainDrawer, backupDrawer, combinedStrand);
