...
public class RnapolisBasePairAnalyzer extends BasePairAnalyzer {
    ...
    @Autowired
    public RnapolisBasePairAnalyzer(@Value("${rnapdbee.adapters.global.rnapolis.path}") String pathToRnapolisAdapter,
                                    @Autowired @Qualifier("adaptersWebClient") WebClient adaptersWebClient) {
        super(adaptersWebClient, pathToRnapolisAdapter);
    }
}
