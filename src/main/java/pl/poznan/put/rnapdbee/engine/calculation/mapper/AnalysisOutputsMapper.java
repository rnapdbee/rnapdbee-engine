...
public class AnalysisOutputsMapper {
    ...
    public Output2D mapToOutput2D(List<AnalysisOutput> analysisOutputs) {
        var singleAnalysisOutputs =
                analysisOutputs.stream()
                        .map(this::mapSingleAnalysisOutputToSecondaryModelAnalysisOutput)
                        .collect(Collectors.toList());
        return new Output2D().withAnalysis(singleAnalysisOutputs);
    }
     ...
     */
    private SingleSecondaryModelAnalysisOutput
            mapSingleAnalysisOutputToSecondaryModelAnalysisOutput(AnalysisOutput analysisOutput) {
        return new SingleSecondaryModelAnalysisOutput()
                .withBpSeq(
                        analysisOutput.bpSeq().entries().stream()
                                .map(BpSeq.Entry::toString)
                                .collect(Collectors.toList()))
                .withStrands(
                        analysisOutput.dotBracket().strands().stream()
                                .map(this::mapStrandIntoSingleStrandOutput)
                                .collect(Collectors.toList()))
                .withCt(
                        analysisOutput.ct().entries().stream()
                                .map(Ct.ExtendedEntry::toString)
                                .collect(Collectors.toList()))
                .withInteractions(
                        analysisOutput.getInterStrand().stream()
                                .map(interStrand -> interStrand.basePair().toString())
                                .collect(Collectors.toList()))
                .withStructuralElement(
                        mapStructuralElementFinderIntoStructuralElementOutput(
                                analysisOutput.structuralElementFinder()))
                .withImageInformation(
                        mapSecondaryStructureImageIntoImageInformationOutput(
    ...
    private StructuralElementOutput mapStructuralElementFinderIntoStructuralElementOutput(
        ...
        return new StructuralElementOutput()
                .withStems(
                        structuralElementFinder.getStems().stream()
                                .map(StructuralElement::toString)
                                .collect(Collectors.toList()))
                .withLoops(
                        structuralElementFinder.getLoops().stream()
                                .map(StructuralElement::toString)
                                .collect(Collectors.toList()))
                .withSingleStrands(
                        structuralElementFinder.getSingleStrands().stream()
                                .map(StructuralElement::toString)
                                .collect(Collectors.toList()))
                .withSingleStrands5p(
                        structuralElementFinder.getSingleStrands5p().stream()
                                .map(StructuralElement::toString)
                                .collect(Collectors.toList()))
                .withSingleStrands3p(
                        structuralElementFinder.getSingleStrands3p().stream()
                                .map(StructuralElement::toString)
                                .collect(Collectors.toList()));
    }

    private ImageInformationOutput mapSecondaryStructureImageIntoImageInformationOutput(
