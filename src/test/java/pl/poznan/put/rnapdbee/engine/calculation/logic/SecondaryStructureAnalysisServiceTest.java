...
class SecondaryStructureAnalysisServiceTest {
    ...
    @Autowired SecondaryStructureAnalysisService cut;

    @BeforeEach
    void provideMocks() {
        SecondaryStructureImage imageMock = Mockito.mock(SecondaryStructureImage.class);
        Mockito.when(imageService.provideVisualization(Mockito.any(), Mockito.any()))
                .thenReturn(imageMock);
    }

    @ParameterizedTest
    @CsvFileSource(resources = "/dotBracketToImageTestCases.csv")
    public void testDotBracketNotationFileAnalysis(
        ...
        var actual =
                cut.analyseDotBracketNotationFile(
    ...
    public void testSecondaryStructureFileAnalysis(
            StructuralElementsHandling structuralElementsHandling,
            VisualizationTool visualizationTool,
            String filename,
            ...
            @AggregateWith(AnalysisOutputTestInformationAggregator.class)
                    List<AnalysisOutputTestInformation> expectedInformationList) {
        var actual =
                cut.analyseSecondaryStructureFile(
